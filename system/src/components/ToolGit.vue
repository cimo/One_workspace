    ...
    methods: {
        init(windowComponent) {
            ...
            if (currentWindowElement !== null) {
                this.elementPart1Container = windowComponent.querySelector(
                    ".part_1_container"
                );
                this.elementPart2Container = windowComponent.querySelector(
                    ".part_2_container"
                );
                this.selectEdit = windowComponent.querySelector(
                    ".part_1_container select[name='edit']"
                );
                this.elementProjectLabel = windowComponent.querySelector(
                    ".part_2_container .project_label"
                );
                this.inputUrl = windowComponent.querySelector(
                    ".part_1_container input[name='url']"
                );
                this.inputUsername = windowComponent.querySelector(
                    ".part_1_container input[name='username']"
                );
                this.inputPassword = windowComponent.querySelector(
                    ".part_1_container input[name='password']"
                );
                this.inputBranchName1 = windowComponent.querySelector(
                    ".part_1_container input[name='branchName']"
                );
                this.inputBranchName2 = windowComponent.querySelector(
                    ".part_2_container input[name='branchName']"
                );
                this.inputCommitDescription = windowComponent.querySelector(
                    ".part_2_container input[name='commitDescription']"
                );
                this.elementOutput1 = windowComponent.querySelector(
                    ".part_1_container .output"
                );
                this.elementOutput2 = windowComponent.querySelector(
                    ".part_2_container .output"
                );
                            ...
                            for (const value of outSplit) {
                                if (
                                    ...
                                    option.text = value.replace(
                                        Config.setting.systemData.extensionGit,
                                        ""
                                    );
        ...
        clickLogic(event) {
            ...
            if (currentWindowElement !== null) {
                const menuElement = Helper.findParent(event.target, [
                    "menu_git"
                ]);
                ...
                if (
                    ...
                    if (this.inputUrl.value.indexOf("https://") !== -1) {
                        const inputUrlValue = this.inputUrl.value.replace(
                            "https://",
                            ""
                        );
                    ...
                    const branchNameMatch1 = /^[A-Za-z0-9-_/ ]+$/.test(
                        this.inputBranchName1.value
                    );
                    const branchNameMatch2 = /^[A-Za-z0-9-_/ ]+$/.test(
                    ...
                    if (event.target.classList.contains("clone") === true)
                        ...
                        event.target.classList.contains("fetch") === true
                    )
                        command = `cd "${this.projectPath}" && git fetch --all`;
                    else if (
                        event.target.classList.contains("reset") === true
                    ) {
                        if (branchNameMatch1 === true)
                        ...
                        event.target.classList.contains("status") === true
                    ) {
                        command = `cd "${this.projectPath}" && git status`;
                    } else if (
                        event.target.classList.contains("commit") === true
                    ) {
                        if (this.inputCommitDescription.value !== "") {
                            ...
                            this.inputCommitDescription.style.borderColor =
                                "#ff0000";
                    } else if (
                        event.target.classList.contains("push") === true
                    ) {
                    ...
                    Sio.readMessage("t_exec_o_gitCommand", data => {
                        ...
                        if (
                            result !== undefined &&
                            event.target.classList.contains("clone") === true
                        ) {
                            buffer = result;
                            ...
                            result !== undefined &&
                            event.target.classList.contains("clone") === false
                        )
                            buffer += result;
                        ...
                        if (data.close !== undefined) {
                            ...
                            if (
                                getComputedStyle(
                                    this.elementPart1Container,
                                    null
                                ).display === "block"
                            )
                                this.elementOutput1.innerHTML = buffer;
                            else this.elementOutput2.innerHTML = buffer;

                            if (
                                event.target.classList.contains("clone") ===
                                    true &&
                                data.close !== 128
                            )
                                this.elementOutput1.innerHTML = "";
                            else if (
                                event.target.classList.contains("fetch") ===
                                    true &&
                                data.close === 0
                            )
                                this.elementOutput1.innerHTML = "";
            ...
            if (currentWindowElement !== null) {
                if (event.target.classList.contains("edit") === true) {
                    if (this.selectEdit.selectedIndex > 0) {
                        ...
                        Sio.readMessage(
                            "t_exec_stream_o_gitChangeLogicEdit",
                            data => {
        ...
        createFile(name, path) {
            ...
            Sio.sendMessage("t_crypt_encrypt_i", {
                tag: "gitSetting",
                text:
                    this.inputPassword !== null ? this.inputPassword.value : ""
            });
            ...
            Sio.readMessage("t_crypt_encrypt_o_gitSetting", data => {
                ...
                if (this.selectEdit !== null) {
                    Sio.readMessage(
                        "t_exec_stream_o_gitClickLogicSave",
                        data => {
                            if (data.chunk === "end") {
                                Sio.stopRead(
                                    "t_exec_stream_o_gitClickLogicSave"
        ...
        deleteOption() {
            if (this.selectEdit !== null) {
                for (const option of this.selectEdit.options) {
                    if (
                        option.value ===
                        `${this.projectName}${Config.setting.systemData.extensionGit}`
                    ) {
                        option.remove();
