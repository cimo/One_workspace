    ...
    methods: {
                        ...
                        if (result !== undefined) {
                            ...
                            if (result.length > 1) {
                                this.windowComponentList[
                                    ...
                                    ".data_component .square_1 .value"
                                ).innerHTML = result[0];
                                this.windowComponentList[
                                    containerName
                                ].querySelector(
                                    ".data_component .square_2 .value"
                                ).innerHTML = result[1];
                                this.windowComponentList[
                                    containerName
                                ].querySelector(
                                    ".data_component .square_3 .value"
                                ).innerHTML = result[2];
                                this.windowComponentList[
                                    containerName
                                ].querySelector(
                                    ".data_component .square_4 .value"
                                ).innerHTML = result[3];
                            }
                        }
                    }
        ...
        close(windowComponent) {
            ...
            if (
                ...
                Sio.stopRead(
                    `t_exec_o_${currentWindowElement.containerName}_data`
                );
