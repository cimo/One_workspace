        ...
        <div class="menuRoot_container">
            <div class="menuRoot_side">
                <div class="menuRoot_side_container">
                    <div
                        v-for="(value, key) in menuRoot.sideItems"
                    ...
                    >
                        <div class="item">
                            <img
                                class="icon"
                                v-bind:src="value.imagePath"
                                v-bind:alt="value.imageName"
                            />
                        </div>
                    </div>
                </div>
            ...
            <div class="menuRoot_panel">
                ...
                <div
                    v-for="(value, key) in menuRoot.projectItems"
                    v-bind:key="`${key}-${value.name}`"
                    class="window_opener"
                    v-bind:data-category="value.category"
                    v-bind:data-name="value.name"
                ...
                <div
                    v-for="(value, key) in menuRoot.toolItems"
                    v-bind:key="`${key}-${value.name}`"
                    class="window_opener"
                    v-bind:data-category="value.category"
                    v-bind:data-name="value.name"
...
export default {
    ...
    methods: {
        clickLogic(event) {
            const menuRootComponent = Helper.findParent(event.target, [
                "menuRoot_component"
            ]);

            if (menuRootComponent !== null) {
                this.menuRootContainer = menuRootComponent.querySelector(
                    ".menuRoot_container"
                );
                ...
                if (
                    event.target.classList.contains("menuRoot_component") ===
                        true ||
                    event.target.classList.contains("menuRoot_image") === true
                ) {
                    if (
                        this.menuRootContainer.style.display === "" ||
                        this.menuRootContainer.style.display === "none"
                    )
                        this.menuRootContainer.style.display = "block";
                    else this.menuRootContainer.style.display = "none";
                }
                ...
                if (windowOpener !== null) {
                    ...
                    if (name === "VueJs") {
                        const tab = window.open(
                            `http://localhost:${Config.setting.vueJs.uiPort}`,
                            "_blank"
                        );
